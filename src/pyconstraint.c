...
int _ped_Constraint_compare(_ped_Constraint *self, PyObject *obj)
{
    ...
    if ((_ped_Alignment_Type_obj.tp_richcompare(self->start_align, comp->start_align, Py_EQ))
        ...
        && (self->min_size == comp->min_size) && (self->max_size == comp->max_size)) {
        return 0;
    } else {
        return 1;
    }
...
PyObject *_ped_Constraint_richcompare(_ped_Constraint *a, PyObject *b, int op)
{
    ...
    if (op == Py_EQ || op == Py_NE) {
        rv = _ped_Constraint_compare(a, b);
...
{
    static char *kwlist[] = {"start_align", "end_align", "start_range", "end_range", "min_size", "max_size", NULL};
    ...
    constraint = ped_constraint_new(start_align, end_align, start_range, end_range, self->min_size, self->max_size);
...
PyObject *py_ped_constraint_new_from_min_max(PyObject *s, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O!O!", &_ped_Geometry_Type_obj, &in_min, &_ped_Geometry_Type_obj, &in_max)) {
        return NULL;
    }
...
PyObject *py_ped_constraint_intersect(PyObject *s, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O!", &_ped_Constraint_Type_obj, &in_constraintB)) {
        return NULL;
    }
